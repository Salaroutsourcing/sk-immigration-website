...
```bash
curl -sL https://www.skimmigrationservices.works/robots.txt | head -100
curl -sI https://www.skimmigrationservices.works/llms.txt | head -15
curl -sI -A "GPTBot" https://www.skimmigrationservices.works/answers | head -15
```
...
3. Spot-check that ChatGPT / Perplexity can fetch `https://www.skimmigrationservices.works/llms.txt` and `/trust.html`.
4. Re-run `node scripts/check-ai-ops.mjs --live` after deploy.
5. **Google Business Profile (required for GEO):** set phone to **+92 304 5999859** only, website `https://www.skimmigrationservices.works`, same Satellite Town NAP as `trust.html`, and collect real reviews. Wrong GBP phone blocks brand + AI trust.
